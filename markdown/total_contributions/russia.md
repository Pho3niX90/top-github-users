...
The `public contributions` and `private contributions` by users in Russia on `2025/2/10 10:49 PM UTC`. This list contains users from `Russia` and cities `Moscow` `Saint-petersburg` `Yekaterinburg` `Novosibirsk` `Kazan` `Omsk` `Volgograd` `Chelyabinsk` `Krasnoyarsk`.

There are `138 countries` and `674 cities` can be found [here](https://github.com/Pho3niX90/top-github-users).

There are `787 users`  in Russia. You need at least `71 followers` to be on this list.
