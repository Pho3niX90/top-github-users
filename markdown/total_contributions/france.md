...
The `public contributions` and `private contributions` by users in France on `2025/2/26 12:13 AM UTC`. This list contains users from `France` and cities `Paris` `Marseille` `Lyon` `Bordeaux` `Toulouse` `Strasbourg` `Nice` `Nantes` `Montpellier` `Lille`.

There are `138 countries` and `674 cities` can be found [here](https://github.com/Pho3niX90/top-github-users).

There are `827 users`  in France. You need at least `215 followers` to be on this list.
