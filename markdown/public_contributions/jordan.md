...
The `public contributions` by users in Jordan on `2025/2/28 3:59 PM UTC`. This list contains users from `Jordan` and cities `Amman` `Aqaba` `Zarqa` `Mafraq` `Jerash` `Irbid`.

There are `138 countries` and `674 cities` can be found [here](https://github.com/Pho3niX90/top-github-users).

There are `928 users`  in Jordan. You need at least `8 followers` to be on this list.
